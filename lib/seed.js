...
module.exports = function seed() {
  if (db.ideas.length > 0) return false; // already seeded
  ...
  db.users.push({
    ...
    pass: hashPassword('sihwin2026'),
    teamId: null,
    createdAt: Date.now()
  });

  save();
  console.log('Seeded: ' + db.ideas.length + ' ideas, ' + db.milestones.length + ' milestones, admin=admin@sih.local / sihwin2026');
  return true;
};
